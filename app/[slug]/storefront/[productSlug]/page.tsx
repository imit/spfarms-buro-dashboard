...
export default function ProductDetailPage({
  ...
  const [otherFlowers, setOtherFlowers] = useState<Product[]>([]);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const sMap: Record<number, Strain> = {};
        allStrains.forEach((s) => { sMap[s.id] = s; });
        setStrainMap(sMap);
        setOtherFlowers(
          allProducts.filter(
  ...
  if (!product) {
    return <p className="text-lg text-muted-foreground">Product not found.</p>;
  }
