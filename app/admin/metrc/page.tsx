...
const TAB_COLUMNS: Record<Tab, { key: string; label: string }[]> = {
  ...
  plants: [
    { key: "Label", label: "Tag" },
    { key: "StrainName", label: "Strain" },
    { key: "GrowthPhaseName", label: "Phase" },
...
export default function MetrcPage() {
  ...
  async function fetchData() {
    ...
    try {
      const opts: Record<string, string | number | undefined> = { ...filters };
  ...
  return (
    <div className="space-y-6 px-10">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-semibold">Metrc API Test</h2>
          <p className="text-sm text-muted-foreground">
            Direct queries against the Metrc sandbox API
          </p>
        </div>
        <Button asChild>
